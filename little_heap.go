...
func (h *LittleHeap) Peek() Node {
	return h.binaryTree[1]
}

func (h *LittleHeap) Insert(v Node) {
	if h.n + 1 > len(h.binaryTree) - 1 {
		h.binaryTree = append(h.binaryTree, Node{})
	}
...
func (h *LittleHeap) DelTop() Node {
	max := h.binaryTree[1]
	h.exch(1,h.n)
	h.n -= 1
	h.binaryTree[h.n + 1] = Node{}
	h.sink(1)
